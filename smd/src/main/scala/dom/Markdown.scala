...
object MarkDown {
  ...
  case class Link(children: Seq[Inline], ref: Option[ReferenceId], args: Seq[Expression]) extends MarkDown
  ...
  case class Quoted(children: Seq[Inline], kind: QuoteKind) extends MarkDown
  case class ReferenceId(value: String)
  case class Reference(ref: ReferenceId, args: Seq[Expression]) extends Block
  case class Space() extends MarkDown
  case class Strong(children: Seq[Inline]) extends MarkDown
  case class Symbol(value: String) extends MarkDown
  case class Text(value: String) extends MarkDown
